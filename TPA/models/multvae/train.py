import torch
from models.multvae.dataset import MultVAEModelDataLoader
from models.multvae.model import MultVAEModel
from models.revisit_training import prepare_experiment_dir, ranking_report, save_training_artifacts

def run(config):
    out_dir=prepare_experiment_dir("multvae",config)
    loader=MultVAEModelDataLoader(config)
    model=MultVAEModel(config,loader.num_users,loader.num_items)
    total_epochs=int(config.get("training",{}).get("epochs",1))
    res=model.fit(loader.interaction_matrix(),total_epochs,float(config.get("training",{}).get("lr",1e-3)))
    history=[{"epoch": total_epochs, "loss": res["loss"]}]
    metrics=ranking_report(model,loader.meta,int(config.get("evaluation",{}).get("k",20)),torch.device("cpu"))
    save_training_artifacts(out_dir,model,history,metrics)
    print(metrics)
    return model,metrics
