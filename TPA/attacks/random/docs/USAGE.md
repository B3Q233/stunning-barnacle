...
```powershell
<repo>\.venv\Scripts\python.exe TPA\attacks\random\run.py --mode classify
```
...
```powershell
<repo>\.venv\Scripts\python.exe TPA\attacks\random\run.py --mode data
```
...
```powershell
<repo>\.venv\Scripts\python.exe TPA\attacks\random\run.py --mode model
```

或一条命令跑全流程：

```powershell
<repo>\.venv\Scripts\python.exe TPA\attacks\random\run.py --mode all
```
...
```powershell
<repo>\.venv\Scripts\python.exe TPA\attacks\random\fit.py --config <配置> --skip-train
```
