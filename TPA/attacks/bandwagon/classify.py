"""Bandwagon 攻击 —— 第 1 步：按交互数分类（不再依赖模型嵌入）
...
3. 按交互数降序排名划分三档（与流行度直方图 Hot/Medium-hot/Tail 一致）：
   ...
   - cold:     其余 40% ~ 100%
4. 缓存到 attacks/bandwagon/data/rec_freq/{dataset}/{model}_top{k}.json
...
from attacks.bandwagon.generate import (  # noqa: E402
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Bandwagon 交互数分类")
    parser.add_argument(
        "--config", type=str,
