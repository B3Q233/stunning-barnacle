...
其中"流行物品"按**训练集交互次数**划分：
统计每个物品在训练集中的交互次数；交互数最高的前 5% 为流行物品，5%~40% 为
普通物品，其余 40%~100% 为冷门物品（与流行度直方图 Hot/Medium-hot/Tail 分层一致）。
...
| filler_size K | 20 | 【AI推断补全】 | 从训练集交互数前 5% 的流行物品池随机采样 K 个 |
...
- 【AI推断补全】流行/普通/冷门的三档划分（前 5% / 5~40% / 其余）来自用户确认的攻击流程，
  非论文原文；`popular_ratio` / `medium_ratio` 可在配置中调整
