...
不修改 models/* 下任何代码，只通过注册表 import 复用。
用法:
  python attacks/pgd/fit.py --config attacks/pgd/config.yaml
"""
...
from attacks.pgd.registry import (
    get_dataset_cls,
    get_model_cls,
    load_model_config,
)
from attacks.pgd.generate import (
    DEFAULT_OUT_DIR as POISONED_OUT_DIR,
    DEFAULT_RAW_META as RAW_META_PATH,
    load_meta,
    load_yaml_config,
)
from attacks.pgd.evaluate import compare_models, ranking_scores, save_report
...
def main(config: Dict[str, Any], skip_train: bool = False,
    ...
    out_root = Path(config.get("output", {}).get("dir", "attacks/pgd/outputs"))
    poisoned_base = PROJECT_ROOT / str(POISONED_OUT_DIR).format(
        dataset=dataset, model=model_name)

    # run_tag 优先级：CLI --tag > config.run_tag > 中毒数据 latest.json > 当前时间
    ...
    if not poisoned_path.exists():
        raise FileNotFoundError(
            ...
            f"或检查 attacks/pgd/data/poisoned/{dataset}/{model_name}/ 下已有的 tag"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PGD 中毒模型拟合")
