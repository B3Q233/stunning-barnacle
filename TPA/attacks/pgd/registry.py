...
满足"选择不同模型（MF / LightGCN）进行投毒"的需求：攻击配置 ``model.name``
指定受害推荐模型，本模块负责把名字解析为模型类 + 默认超参配置。
新增模型时，只需在 AVAILABLE_MODELS 中登记一条：

    "模型名": {
        "model_cls": "包.模块:类名",          # 必填
        "dataset_cls": "包.模块:类名",        # 可选，训练数据加载器
        "config_path": "模型自身 config.yaml 相对 TPA 根的路径",
        "description": "一句话说明",
    }

约定：被登记模型需实现与 LightGCN 相同的接口：
...
AVAILABLE_MODELS: Dict[str, Dict[str, Any]] = {
    "lightgcn": {
        ...
        "description": "LightGCN (SIGIR 2020)：多层 LGC + 层组合 + BPR",
    },
    "mf": {
...
def get_model_entry(name: str) -> Dict[str, Any]:
    if name not in AVAILABLE_MODELS:
        raise ValueError(
            ...
            f"如需新增，请在 attacks/pgd/registry.py 的 AVAILABLE_MODELS 中登记。"
        )
