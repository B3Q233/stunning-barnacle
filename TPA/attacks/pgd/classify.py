"""PGD 攻击 —— 第 1 步：模型推荐频次分类

与 bandwagon 模板完全一致：
1. 加载干净模型 checkpoint（模型由 config ``model.name`` 指定）
2. 计算全量评分矩阵（按批），对每个用户取 Top-K 推荐物品（过滤训练集已交互，
   与评估协议一致）
3. 统计每个物品在 Top-K 中出现的次数
4. 按频次划分三档：流行（前 20%）/ 普通 / 冷门
5. 缓存到 attacks/pgd/data/rec_freq/{dataset}/{model}_top{k}.json
...
from attacks.pgd.fit import build_training_config
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PGD 推荐频次分类")
    parser.add_argument(
        "--config", type=str,
