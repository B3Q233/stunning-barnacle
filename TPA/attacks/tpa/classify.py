...
def resolve_active_checkpoint(config: Dict[str, Any]) -> Path:
    """路径构造 / 频次分类使用的 checkpoint。

    代理模式（surrogate.enabled=true）用 surrogate.checkpoint（攻击者自己的模型），
    否则用受害模型（白盒对照）。
    """
...
def main(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ckpt = resolve_active_checkpoint(config)
    ...
    print(f"[classify] 模型={model_name}（{mode}），checkpoint={ckpt}，"
