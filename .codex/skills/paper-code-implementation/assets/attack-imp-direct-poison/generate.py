...
3. 为每个目标构造假用户画像：[K 个流行物品 + 目标物品]
   ...
   TODO: 换攻击语义（如随机画像 / 平均画像 / 多跳路径画像）时只需改
   generate_fake_profiles 一处，其余流程不变
4. 注入训练集 → 产出 poisoned meta.pkl + 注入统计

用法:
  python attacks/attack_imp_direct_poison/generate.py --config attacks/attack_imp_direct_poison/config.yaml
"""
...
def load_rec_freq_cache(config: Dict[str, Any], model_name: str, k: int,
    ...
    from attacks.attack_imp_direct_poison.classify import load_cache
    ...
    if strategy == "specified":
        ...
        if categories is None:
            raise FileNotFoundError(
                ...
                "请先运行 python attacks/attack_imp_direct_poison/run.py --mode classify"
            )
