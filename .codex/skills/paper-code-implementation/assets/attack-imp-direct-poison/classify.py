...
5. 缓存到 attacks/{attack.name}/data/rec_freq/{dataset}/{model}_top{k}.json，
   供 generate.py 选择目标 / 采样 filler 使用

用法:
  python attacks/attack_imp_direct_poison/classify.py --config attacks/attack_imp_direct_poison/config.yaml
"""
...
from attacks.attack_imp_direct_poison.registry import get_model_cls
from attacks.attack_imp_direct_poison.generate import (
    load_meta,
    load_yaml_config,
    raw_meta_path,
)
from attacks.attack_imp_direct_poison.fit import build_training_config
...
def load_cache(config: Dict[str, Any], model_name: str, k: int,
    ...
    if not path.exists():
        if required:
            raise FileNotFoundError(
                f"推荐频次分类不存在: {path}\n"
                f"请先运行: python attacks/attack_imp_direct_poison/run.py --mode classify"
            )
        return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="推荐频次分类（攻击模板第 1 步）")
    parser.add_argument(
        "--config", type=str,
