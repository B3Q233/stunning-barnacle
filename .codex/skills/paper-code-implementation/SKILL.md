...
---
  ...
  **使用规则**：
  ...
  - **注释完整性（强制）**：配置文件的每个参数与代码注释都必须涵盖——为什么这样做（设计动机/取舍）、功能是什么、参考的公式是什么（如有，注明公式编号或论文/官方代码出处）、使用举例（典型取值或调用方式）。涉及复杂逻辑与矩阵运算的代码，注释必须给出矩阵变换过程（输入形状 → 中间张量形状 → 输出形状、行列含义）与对应逻辑，禁止无解释的矩阵操作。注释默认中文，来源标注沿用 `[paper]/[ai]/[unreported]/[官方代码]`。
