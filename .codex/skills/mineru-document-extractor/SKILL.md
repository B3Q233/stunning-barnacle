...
description: >
  MinerU document extraction: convert PDFs, scanned documents, images, Word (DOC/DOCX),
  PowerPoint (PPT/PPTX), Excel (XLS/XLSX), and web pages into clean Markdown, HTML, LaTeX,
  or DOCX via the mineru-open-api CLI. Use when the user asks to extract text from a PDF,
  convert a PDF/Word/Excel/PPT to Markdown, parse an academic paper with tables and
  formulas, OCR a scanned document, batch convert PDFs, crawl a web page to Markdown, or
  extract tables from a document. Two modes: flash-extract (token-free, instant, Markdown
  only, files under 10 MB / 20 pages) and extract (VLM-based layout analysis, multi-format
  output, batch mode, requires a MinerU token). Supports 80+ languages including Chinese,
  English, Japanese, Korean, Arabic, and more. Requires the mineru-open-api CLI (npm or Go).
---
